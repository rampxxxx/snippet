...
vim.opt.cursorline = false -- Only generates confusion
...
vim.keymap.set("n", "<leader><F8>", "<cmd>NvimTreeToggle<CR>")
...
vim.keymap.set("n", "gT", "<cmd>tabprev<CR>")
-- END : Personal keymaps
...
require("lazy").setup({
    ...
    {
        "rose-pine/neovim",
        -- Commented as need setup , assign colorscheme order at bottom
        -- init = function()
    ...
    "nvim-neotest/nvim-nio", --Required by nvim-dap-ui 11-04-2024
    "rcarriga/nvim-dap-ui", -- Open several windows to show local vars, stack, dap console,...
    "theHamsta/nvim-dap-virtual-text", --Show floating var values while debugging.
    ...
    { -- Fuzzy Finder (files, lsp, etc)
        ...
        config = function()
            ...
            vim.keymap.set("n", "<leader>th", builtin.help_tags, { desc = "[T]earch [H]elp" })
            vim.keymap.set("n", "<leader>tk", builtin.keymaps, { desc = "[T]earch [K]eymaps" })
            vim.keymap.set("n", "<leader>tf", builtin.find_files, { desc = "[T]earch [F]iles" })
            vim.keymap.set("n", "<leader>tb", builtin.builtin, { desc = "[T]earch [T]elect Telescope" })
            vim.keymap.set("n", "<leader>tw", builtin.grep_string, { desc = "[T]earch current [W]ord" })
            vim.keymap.set("n", "<leader>tg", builtin.live_grep, { desc = "[T]earch by [G]rep" })
            vim.keymap.set("n", "<leader>ts", builtin.lsp_dynamic_workspace_symbols, { desc = "[T]earch by [S]ymbols" })
            vim.keymap.set("n", "<leader>tc", builtin.git_bcommits, { desc = "[T]earch by Buffers [C]ommits" })
            vim.keymap.set("n", "<leader>tt", builtin.treesitter, { desc = "[T]earch by [T]reesitter names" })
            vim.keymap.set("n", "<leader>td", builtin.diagnostics, { desc = "[T]earch [D]iagnostics" })
            vim.keymap.set("n", "<leader>tr", builtin.resume, { desc = "[T]earch [R]esume" })
            vim.keymap.set("n", "<leader>t.", builtin.oldfiles, { desc = '[T]earch Recent Files ("." for repeat)' })
    ...
    { -- LSP Configuration & Plugins
        ...
        config = function()
            ...
            -- 1. Set up Mason and Mason-LSPConfig
            require("mason").setup()
            require("mason-lspconfig").setup({
                -- Optional: list servers to ensure they are installed
                ensure_installed = {
                    "terraformls",
                    "tflint",
                    "gopls",
                ...
                -- Optional: automatic_enable is true by default and will call vim.lsp.enable()
                -- for all servers installed by Mason.
                automatic_enable = true,
            })

            -- 2. Configure individual LSP servers with vim.lsp.config()
            -- This is where you put your specific server options.
...
vim.cmd.colorscheme("rose-pine")
-- END : rose-pine config and load in that order
-- INIT : nvim-lint
...
require("dapui").setup() -- nvim-dap-ui config.
require("nvim-dap-virtual-text").setup() -- Floating/Virtual information of vars inlined.
...
require("conform").formatters.golines = {
    inherit = false,
    command = "golines",
    args = { "-m", "150", "--shorten-comments", "-t", "4" },
}
require("conform").formatters.c = { -- Here : Don't know if lang name or fmt cmd name
    inherit = false,
    command = "clang-format",
    args = { "-style=file" }, -- Use clang-format conf file
}
